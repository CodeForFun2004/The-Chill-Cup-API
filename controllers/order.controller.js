...
exports.getAllOrders = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Không thể lấy danh sách đơn hàng hệ thống" });
  }
};
...
exports.getStaffOrders = async (req, res) => {
  try {
    ...
    if (!store) {
      return res
        .status(404)
        .json({ error: "Nhân viên chưa được gán quản lý cửa hàng nào" });
    }
  ...
  try {
    ...
    if (assignShipperId) {
      ...
      const shipper = await User.findOne({
        staffId: assignShipperId,
        role: "shipper",
      });
      if (!shipper) {
        return res
          .status(404)
          .json({ error: "Không tìm thấy shipper với mã nhân viên này" });
      }
...
exports.getShipperOrders = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Không thể lấy danh sách đơn hàng của shipper" });
  }
};
...
exports.completeDeliveryByShipper = async (req, res) => {
  try {
    ...
    if (!order)
      return res
        .status(404)
        .json({ error: "Đơn hàng không thuộc shipper này" });
